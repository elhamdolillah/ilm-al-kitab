#!/usr/bin/env python3
"""
MAL Code Generator v2: استخراج كيانات ذكي + حفظ ملفات .mal
الهدف: توليد كود MAL نقي 100% وحفظه في ملفات لرفع نسبة MAL في الكود المصدري.
"""
import torch
...
class MALCodeGenerator:
    def __init__(self, model_path='MHLJ_Fusion_Head_100pct.pt'):
        ...
        # مجلد حفظ ملفات MAL المولدة
        self.output_dir = os.path.join(current_dir, 'generated_mal')
        os.makedirs(self.output_dir, exist_ok=True)
        print("✅ مولد كود MAL v2 جاهز!\n")
    def predict(self, text):
        inp = torch.tensor([self.mal_tok.encode(text)])
        with torch.no_grad():
            emb = self.mal_model.tok_emb(inp)
            i_out, e_out = self.head(emb)
        return (self.intents[torch.argmax(i_out, -1).item()], 
                self.entities[torch.argmax(e_out, -1).item()])
    def extract_entities(self, text):
        """استخراج ذكي للمتغيرات والأرقام والدوال من النص العربي"""
        entities = {
            'variables': [],      # المتغيرات العربية (س، ص، ع، ...)
            'numbers': [],         # الأرقام
            'operators': [],       # العوامل (+، -، =، >، <)
            'functions': [],       # أسماء الدوال
            'theorem_name': None   # اسم النظرية المقترح
        }
        # 1. استخراج المتغيرات العربية المنفردة
        arabic_vars = re.findall(r'\b([س-ي])\b', text)
        entities['variables'] = list(set(arabic_vars))
        # 2. استخراج الأرقام
        numbers = re.findall(r'\b(\d+(?:\.\d+)?)\b', text)
        entities['numbers'] = numbers
        # 3. استخراج العوامل الرياضية
        operators = re.findall(r'([+\-*/=<>≤≥≠])', text)
        entities['operators'] = list(set(operators))
        # 4. استخراج أسماء الدوال (كلمات تنتهي بـ "دالة" أو تتبعها)
        func_match = re.search(r'دالة\s+(\w+)', text)
        if func_match:
            entities['functions'].append(func_match.group(1))
        # 5. توليد اسم نظرية من النص
        if 'أثبت' in text or 'برهن' in text:
            if '=' in text:
                entities['theorem_name'] = 'equality_theorem'
            elif '>' in text or '<' in text:
                entities['theorem_name'] = 'inequality_theorem'
            else:
                entities['theorem_name'] = 'general_theorem'
        return entities
    def generate(self, text, save_to_file=True):
        """توليد كود MAL نقي مع استخراج ذكي للكيانات"""
        intent, entity = self.predict(text)
        ents = self.extract_entities(text)
        print(f"🔍 التحليل: النية = [{intent}] | الكيان = [{entity}]")
        print(f"📊 الكيانات المستخرجة: {ents}")
        # توليد كود MAL بناءً على النية والكيانات
        if intent == 'define_var':
            var_name = ents['variables'][0] if ents['variables'] else 'x'
            mal_code = f"""-- تعريف متغير مولد آلياً بواسطة MHLJ
def {var_name} : Type := 
  -- القيمة الافتراضية، يمكن تعديلها حسب المتطلب
  default
"""
        elif intent == 'eval_expr':
            # بناء تعبير رياضي من الأرقام والعوامل
            if ents['numbers'] and ents['operators']:
                expr = ' '.join(ents['numbers'][:2])
                if len(ents['operators']) > 0:
...
"""
            ...
            stmt = text.replace('أثبت أن', '').replace('برهن أن', '').strip()
            mal_code = f"""-- نظرية مولدة آلياً بواسطة MHLJ
theorem {theorem_name} : ∀ {vars_str}, {stmt}
proof
  -- استراتيجية البرهان: استقراء أو تطابق
  -- يمكن توسيعها باستخدام tactics متقدمة
  admit
qed
"""
...
-- الرجوع إلى مكتبة MAL القياسية للحصول على التعريف والخصائص
"""
        # حفظ في ملف .mal
        if save_to_file:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f"generated_{intent}_{timestamp}.mal"
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("🧪 اختبار مولد كود MAL v2 (مع استخراج ذكي للكيانات)")
    print("="*70)
    for text in test_cases:
        print(f"\n📝 المدخل: '{text}'")
        mal_code = generator.generate(text)
        print("📤 المخرج (MAL Code):")
        print("-" * 40)
        print(mal_code)
        print("-" * 40)
    print(f"\n✅ تم توليد {len(test_cases)} ملفات MAL في: {generator.output_dir}")
