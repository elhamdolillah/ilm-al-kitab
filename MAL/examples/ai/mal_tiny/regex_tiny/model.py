...
class RegexTinyModel(nn.Module):
    def __init__(self, vocab_size=122):
        ...
        self.layers = nn.ModuleList([TransformerBlock(self.embed_dim, self.num_heads) for _ in range(self.num_layers)])
        self.ln_f = nn.LayerNorm(self.embed_dim)
        self.head = nn.Linear(self.embed_dim, vocab_size, bias=False)
        print(f"🏗️ Regex-Tiny: {sum(p.numel() for p in self.parameters()):,} params")
